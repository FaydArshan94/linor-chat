...
router.get("/debug-key/:key", async (req, res) => {
  const hash = crypto.createHash("sha256").update(req.params.key).digest("hex");
  const tenant = await Tenant.findOne({ apiKey: hash });
  res.json({ hash, found: !!tenant, isActive: tenant?.isActive });
});
